...
The agent reproduces a failed state. The agent restarts the app, reloads the Metro bundle, grants or resets a permission, or opens a URL. On a Chromium app only the URL is the agent's: the user quits the app, and the agent starts an Electron app again itself. Then the agent does the steps again, and reads the logs and the component tree after each step.
