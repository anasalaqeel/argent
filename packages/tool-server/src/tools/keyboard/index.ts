  ...
  clear: z
    ...
    .describe(
      "Empty the focused text field before typing. Use this whenever a field may already hold a value — typing alone does NOT replace it: the old text survives and the new text goes in at the caret, which lands after the old value or splices into the middle of it depending on where focus left the caret. " +
        ...
        "On Android the read-back needs the screen's view hierarchy and measures focused `EditText` nodes only; where the screen cannot be captured, or the focused field is one it cannot read (a password box, a WebView input, a custom widget), the clear stays best-effort like iOS. The repair itself deletes backwards from end-of-LINE, so a multi-line field keeps what sits below the caret. Levels older than `input keycombination` take that same end-of-LINE delete from the start, and there a field over 150 characters is refused rather than partly deleted; the same refusal can fire on a level that HAS the subcommand when the read-back leaves more residue than backspaces are allowed to clear. On those older levels a length that cannot be read at all (a password field, or a screen the device would not capture) falls back to a fixed 158 backspaces — more than every length that path accepts, so a field only keeps its head past 158 characters."
    ),
  delayMs: z
    .number()
