...
Returns { cookies, count } for get, or a small status object ({ set } / { deleted } / { cleared }) otherwise. Fails if the device is not a Chromium (CDP) device, or set is missing name/value. Like every Chromium tool it resolves through the active page, so an app that is up with no open tab/window fails before it runs; ask the user to reopen a window. Chromium-only.`,
