...
A Chromium device may have several tabs / BrowserWindows. Use `chromium-tabs` (which needs an existing page, so it cannot reopen the last window once it is closed) to `list` them (stable ids `t1`, `t2`, …, optional labels), open a `new` one, `select` which is active, or `close` one.
