...
report stops the batch and counts the remaining flows skipped. So does a refusal
about the CALL rather than the file — every flow in a batch runs with the
same arguments apart from its own path, so a bad --env name is one fault,
not one per file.
